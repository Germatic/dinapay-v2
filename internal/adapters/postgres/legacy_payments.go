...
func (s *Store) ListDashboardPayments(ctx context.Context, accountID, merchantID string, options core.PaymentListOptions) (core.PaymentPage, error) {
	page, err := s.listConsolidated(ctx, dashboardConsolidatedSelect, accountID, merchantID, options)
...
func (s *Store) listConsolidated(ctx context.Context, query, accountID, merchantID string, options core.PaymentListOptions) (core.PaymentPage, error) {
	...
	for rows.Next() {
		var p core.Payment
		var customer, payer, metadata, paymentData, pricing, failure []byte
		var origin string
		var creation time.Time
		var expiration, confirmation *time.Time
		if err := rows.Scan(&p.TransactionID, &p.AccountID, &p.MerchantID, &p.ExternalID, &p.Status, &p.Amount, &p.ReceivedAmount, &p.Currency, &p.PaymentMethod, &p.Description, &creation, &expiration, &confirmation, &p.ActionURL, &customer, &payer, &metadata, &paymentData, &pricing, &failure, &origin); err != nil {
			return core.PaymentPage{}, err
		}
...
const consolidatedSelect = `WITH all_payments AS (
	...
	CASE WHEN p.provider='binancepay' THEN 'crypto_payment' WHEN p.currency='BRL' AND COALESCE(p.qr_data,'')<>'' THEN 'qr' ELSE 'bank_transfer' END,
	COALESCE(p.description,''),p.created_at,p.expiration,p.received_at,COALESCE(p.action_url,''),p.customer,NULL::jsonb,p.metadata,
	...
	NULL::jsonb,'v1'::text FROM payments p LEFT JOIN merchants m ON m.id=p.merchant_id
